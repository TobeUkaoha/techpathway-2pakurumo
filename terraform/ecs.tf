  ...
  container_definitions = jsonencode([
    {
      ...
      essential   = true
      healthCheck = {
        command     = ["CMD-SHELL", "wget -qO- http://localhost:${var.frontend_port}/ || exit 1"]
        interval    = 30
        timeout     = 5
        retries     = 3
        startPeriod = 10
...
resource "aws_ecs_service" "backend" {
  ...
  # Allow new deployments without downtime
  ...
  # Ignore task definition changes — CI/CD handles updates
  lifecycle {
